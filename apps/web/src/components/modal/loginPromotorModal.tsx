...
export default function LoginPromotorModal() {
    ...
    const handleLogin = async (dataSet: {email: string, password: string}) => {
        try {
            const response = await fetch('http://localhost:8000/api/promotors/login', {
                method: "POST",
                headers: {
            ...
            {() => {
                return (
                        <div className='flex flex-col items-center justify-center mx-3 bg-white rounded-2xl'>
                                <h1 className='text-4xl font-bold mb-10 sm:mb-20 text-center text-balance'>Promotors</h1>
                            <Form className='flex flex-col items-center w-full gap-7'>
                                    <div className='w-full'>
                                        <div  className='flex flex-col'>
                                            <label htmlFor="email" className="text-sm">Email</label>
                                            <Field type="email" placeholder="Promotors email" name="email" className="bg-zinc-200 text-xl text-xblack border-b-[1px] focus:outline-none placeholder:text-zinc-300" />
                                        </div>
                                    ...
                                    <div className='w-full'>
                                        <div  className='flex flex-col'>                                            
                                            ...
                                            <Field type="password" placeholder="Promotors password" name="password" className="bg-zinc-200 text-xl border-b-[1px] border-xmetal focus:outline-none placeholder:text-zinc-300" />
                                        </div>
                                        <ErrorMessage component="div" name="password"  className="text-xmetal text-sm text-[0.7rem] fixed" />
                                    </div>                                  
                                <button type="submit" className="text-black text-2xl w-full py-2 rounded-xl mt-10 sm:mt-20 relative">Login</button>
                            </Form>
                        </div>
                )
            }}
